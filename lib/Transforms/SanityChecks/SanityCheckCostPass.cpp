...
bool SanityCheckCostPass::runOnModule(Module &M) {
    ...
    sanitychecks::CostModelAnalysis &CM = getAnalysis<sanitychecks::CostModelAnalysis>();
    ...
    for (Function &F: M) {
        ...
        for (Instruction *Inst: SCI.getSanityCheckBranches(&F)) {
            ...
            for (Instruction *CI: SCI.getInstructionsBySanityCheck(BI)) {
                unsigned CurrentCost = CM.getInstructionCost(CI);
            ...
            MDNode *MD = MDNode::get(M.getContext(), ArrayRef<Value*>(
                    Constant::getIntegerValue(Type::getInt64Ty(M.getContext()), CountInt)));
...
void SanityCheckCostPass::getAnalysisUsage(AnalysisUsage& AU) const {
    ...
    AU.addRequired<sanitychecks::CostModelAnalysis>();
    AU.setPreservesAll();
}
